...
- `cdp_integration.py` — Chrome DevTools Protocol client for Cloak Browser, so a platform (WhatsApp Web, Telegram Web, etc.) logged into a real browser session can be read from / typed into programmatically. Talks to Cloak Browser's CDP endpoint on `127.0.0.1:19222`.
